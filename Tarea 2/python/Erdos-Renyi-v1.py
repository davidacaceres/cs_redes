...
def make_outdir(N):
    ...
    outdir = Path(f"run-e2r/N{N}_{stamp}"); outdir.mkdir(parents=True, exist_ok=True); return outdir
...
rho_kxs = np.linspace(0, 0.6, 7)

SEED = 12021974
RKK_TOL = 0.01
MAX_SWAPS = 10000
REWIRE_FACTOR = 10      # ≈ intentos por arista

# Número de procesos configurable (usa todos los cores por defecto)
NUM_PROCESOS_DESEADOS = 18
NUM_PROCESOS = max(1, min(NUM_PROCESOS_DESEADOS, cpu_count() or 1))
...
def main():
    ...
    logger.log(f"Procesos: deseados={NUM_PROCESOS_DESEADOS}, usados={NUM_PROCESOS}, cpu_count={cpu_count()}")
        ...
        for j, px1 in enumerate(px1_values):
            ...
            for rkk_target in rkk_values:
                ...
                ax.plot(xs, ys, marker="o", linewidth=1.5,
                        label=f"r_kk obj {rkk_target:+.1f} (fin {rkk_fin:+.2f})")
            ax.set_title(f"ER: ⟨k⟩={avg_k}, P(x=1)={px1}")
            ax.set_xlabel("Correlación grado–atributo $\\rho_{kx}$")
            ax.set_ylabel("Fracción nodos en mayoría activa")
            ax.grid(True, alpha=0.3)
            ax.legend(fontsize=9)

    plt.tight_layout()
    plt.suptitle("Ilusión de Mayoría en Redes Erdős–Rényi (Fig. 3) — Paralelo por procesos (curvas)", y=1.02)
